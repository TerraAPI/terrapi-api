...
public class OsmProperties {
    ...
    /**
     * Slim mode. {@code false} (default) keeps the middle in RAM вЂ” faster, but needs several GB of
     * memory; best for dev and full reimports. {@code true} stores the middle on disk
     * ({@code --slim --drop --flat-nodes}) вЂ” memory-gentle for constrained hosts, but slower.
     */
